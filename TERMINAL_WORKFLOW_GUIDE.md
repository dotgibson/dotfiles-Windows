...
> tree, not assumed. Where a config is **vendored** — `nvim/` from `dotfiles-nvim`,
> `starship/starship.toml` and `theme/palette.toml` from `dotfiles-core` — it is
> called out as such: sync those, don't hand-edit.

**Scope note.** This box is the **native-host layer** of a twelve-repo, three-layer
...
### 8.1 Neovim (`nvim/`, **vendored from `dotfiles-nvim`**)

`nvim-sync.ps1` vendors `dotgibson/dotfiles-nvim`'s `nvim/` (incl. `lazy-lock.json`) via
`robocopy /MIR`; a bare run pins that repo's newest `vX.Y.Z` **release**, and the flags are
`-Ref <tag>` (an exact pin), `-FollowBranch` (track `main`'s tip), `-NvimLocal`, `-NvimRemote`.
Provenance is stamped into **`nvim.lock`** at the repo root — outside the tree it describes, so
the parity gate compares `nvim/` with no exclusions. The whole dir is symlinked to
**`%LOCALAPPDATA%\nvim`** — that is where Windows nvim reads config. Namespace `gerrrt`
(`init.lua` = `require("gerrrt")`), **leader = Space**, lazy.nvim plugin manager, netrw disabled
(nvim-tree owns files). *(Do not hand-edit — fix upstream in `dotfiles-nvim` and re-sync.)*
...
*Generated by a read-only audit of `dotfiles-Windows` — the native-host layer of the twelve-repo
system — covering the PowerShell shell, psmux, the tiling desktop, maintenance/packages, and the
editor tiers. `nvim/` is **vendored from `dotfiles-nvim`**, and `starship/starship.toml` and
`theme/palette.toml` **from `dotfiles-core`** (via `nvim-sync.ps1` / `starship-sync.ps1` /
`theme-sync.ps1`): change those upstream in the owning repo and re-sync, never hand-edit here. The same goes for any hex inside a
