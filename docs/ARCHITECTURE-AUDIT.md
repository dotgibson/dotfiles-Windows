...
| -- | ------ | --------- | ------- | --------- | ------ |
| B1 | 🟡 | `nvim-sync.ps1`, parity prose | One-way `robocopy /MIR`, no recorded source ref, no drift detection — parity can silently rot. | Record synced Core SHA (`nvim/.core-ref`) ✅; **add a CI parity check** that fails on divergence ⬜; pin `nvim-sync` to that ref by default ⬜. | High |
...
| B4 | ⬜ | `packages/scoopfile.json`, `winget.json` | Only PS modules are version-pinned; scoop/winget apps float to latest, so two boxes diverge — "reproducible" is partial. | Commit a resolved lockfile (scoop/winget export); add a `-Frozen` install mode that installs exact versions; CI diffs drift. | High |
| B5 | ⬜ | `.github/workflows/ci.yml` | Coverage gate measures only `05-lib.ps1`; the other pure-helper files (incl. the new `Dotfiles/*.Helpers.ps1`) are tested but **ungated**. Floors (`minTotal=120`, `minFiles=11`) are hand-maintained magic numbers. | Expand `CodeCoverage.Path` to the whole pure-helper surface; replace magic floors with a generated/checked-in baseline. | Medium |
| B6 | ⬜ | `profile.ps1` loader | Fragment order/deps are implicit (`NN-` sort + shared globals); no declared contract. | Lightweight per-fragment `requires`/`provides` header + a test asserting the invariant. | Medium |
...
| B9 | ⬜ | `install.ps1` / `uninstall.ps1` / `profile.ps1` | Backslash path literals in `Join-Path` (`'powershell\core\05-lib.ps1'`); a latent footgun on Linux pwsh (where the fast CI gate runs). | Forward slashes (pwsh accepts them everywhere) or `[IO.Path]::Combine`, as `Get-DotfilesLinkPlan` already does. | Medium |
...
| B12 | ⬜ | `ci.yml` lua-lint | `luacheck` is apt/luarocks-installed **unpinned** every run — slow, non-hermetic, a supply-chain gap vs. the SHA-pinned Actions. | Cache the luarocks tree; pin an exact `luacheck` version. | Medium |
