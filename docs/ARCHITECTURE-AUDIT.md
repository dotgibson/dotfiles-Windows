...
| B9 | 🟡 | `install.ps1` / `uninstall.ps1` / `profile.ps1` / `Install-Packages.ps1` | Backslash path literals in `Join-Path` (`'powershell\core\05-lib.ps1'`); a latent footgun on Linux pwsh (where the fast CI gate runs). | `install.ps1`/`uninstall.ps1` normalized to forward slashes ✅ _(PR #13)_; `profile.ps1` (`'PowerShell\Modules'`) and `packages/Install-Packages.ps1` (`'..\powershell\core\05-lib.ps1'`) still carry the pattern ⬜. | Medium |
