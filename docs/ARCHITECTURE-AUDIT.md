...
| U9 | ✅ | `55-help.ps1` `dothelp -i` | The fzf picker hid the preview, so the description/group columns were unused while choosing. | The list is now a clean column of command names; the highlighted row's group + full description fill an `fzf --preview` (`[{3}] {2}`), and `--nth 1,2` keeps fuzzy-search matching the description even though it's no longer in the list. A Help.Tests assertion locks the `command\tdesc\tgroup` field order the preview depends on. | Medium |
