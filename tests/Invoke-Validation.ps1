...
$preEc = $script:fail
# .lock covers nvim.lock, the vendored editor's pin: it is generated by
# nvim-sync.ps1 exactly the way .core-ref is, so it drifts the same way and needs
# the same LF/final-newline gate (dotfiles-core#1124).
$textExt = '.ps1', '.psm1', '.psd1', '.lua', '.json', '.yml', '.yaml', '.toml', '.md', '.lock'
$crlfOk  = '.cmd', '.bat'
